...
import { compressImage, getImageDimensions } from '../utils/imageUtils';
...
const CardDesignerPage: React.FC = () => {
    ...
    const getDimensions = useCallback(() => {
        ...
        return { width: 516, height: 330 }; // 86mm x 55mm scaled by 6
    }, [cardType]);
    ...
    const captureAndUploadImage = async (side: 'front' | 'back', data: CardFaceData, folderPath: string): Promise<string> => {
        ...
        // Short delay to allow any final renders/reflows
        ...
        const dataUrl = await Promise.race([
            htmlToImage.toPng(element, {
                ...
                pixelRatio: 4, // Reduced to 4 (approx 300 DPI) for better stability
                cacheBust: false,
                width: CARD_WIDTH,
                height: CARD_HEIGHT,
                backgroundColor: null,
                style: {
                    // Ensure the captured element behaves correctly
                    transform: 'none',
                    margin: 0,
                }
            }),
        ...
        const { error } = await client.storage.from(BUCKET_CARD_IMAGES).upload(filePath, blob, { upsert: true, cacheControl: '3600' });
    ...
    const containerClasses = cardType === 'standie'
        ...
        : `${previewMode === 'mobile' ? 'w-[95%] max-w-[400px]' : 'w-[90%] md:w-[45%]'} aspect-[86/55] shadow-2xl transition-all duration-300`;
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden relative flex flex-col items-center justify-start p-1 md:p-4 bg-zinc-900/50 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent" onMouseDown={handleDeselect} onTouchStart={handleDeselect}>
                <div className="absolute top-2 left-2 right-2 z-20 flex justify-between items-start pointer-events-none">
                    ...
                    <div className="pointer-events-auto flex flex-col items-end gap-1">
                        ...
                        <input type="file" accept="image/*" className="hidden" ref={uploadDesignRef} onChange={async (e) => {
                            if (e.target.files?.[0]) {
                                const file = e.target.files[0];

                                // 1. Size Check (5MB Limit)
                                if (file.size > 5 * 1024 * 1024) {
                                    showToast("File too large (Max 5MB). Please compress it.", "error");
                                ...
                                try {
                                    ...
                                    if (dims.width < 1000) {
                                        showToast("Warning: Low resolution image. Print quality may be blurry.", "info");
                                    }

                                    // 3. Compression & Load
                                    const compressedBlob = await compressImage(file, 1500, 0.85); // Slightly higher quality for print designs
                                    const reader = new FileReader();
                                    ...
                                    console.error("Error processing design upload:", err);
