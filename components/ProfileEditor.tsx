...
const ProfileEditor: React.FC = () => {
  ...
  const handleSubmit = useCallback(async (isAutosave = false) => {
    ...
    const performSave = async () => {
        ...
        const updates: any = {};
        // Only upload if file objects exist (means changed)
        ...
        // Combine current data with new image URLs
        const finalData = { ...formData, ...updates };
        
        // Update local state with the REAL urls immediately so subsequent auto-saves don't use blob URLs
        ...
        if (id) {
            ...
            if (data) navigate(`/profile/${data.id}/edit`, { replace: true });
        }
        ...
        if (!isAutosave && mountedRef.current) {
    ...
    try {
        // Wrap operation in a race with a timeout
        await Promise.race([
            performSave(),
            new Promise((_, reject) => setTimeout(() => reject(new Error("Save operation timed out. Please check your connection.")), 25000))
        ]);
    } catch (err: any) {
        console.error(err);
        ...
        if (!isAutosave && mountedRef.current) setLoading(false); 
    }
