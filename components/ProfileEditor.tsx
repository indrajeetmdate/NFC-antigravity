...
const ProfileEditor: React.FC = () => {
    ...
    const uploadFile = async (file: File, bucket: string, folderPath: string, baseFileName: string) => {
        ...
        if (file.type.startsWith('image/')) {
            try {
                // Compress to max 1024px, 0.7 quality
                const compressedBlob = await compressImage(file, 1024, 0.7);
                fileToUpload = compressedBlob;
                fileExt = 'jpg'; // We convert to JPEG in utility
                console.log(`Compressed ${file.name}: ${(file.size / 1024).toFixed(2)}KB -> ${(compressedBlob.size / 1024).toFixed(2)}KB`);
            } catch (err) {
        ...
        // Cache for 1 year (31536000 seconds) since filename is unique
        await client.storage.from(bucket).upload(filePath, fileToUpload, {
    ...
    const handleSubmit = useCallback(async (isAutosave = false) => {
        ...
        try {
            ...
            if (mountedRef.current) {
                if (!isAutosave) {
                    showToast('Saved Successfully! Redirecting to payment...', 'success');
                    // Redirect to payment page on manual save
