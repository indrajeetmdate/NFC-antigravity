...
const Auth: React.FC<AuthProps> = ({ type }) => {
  ...
  // Show verifying state if URL has hash or if we are loading session status but think we might be logged in
  if (isVerifying || (profileLoading && !session && type === 'login')) {
      return (
