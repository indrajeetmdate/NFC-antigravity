...
const PaymentPage: React.FC = () => {
    ...
    // Ref to track if the user just completed a payment on this page
    // This prevents the redirect from firing after payment refreshes the profile
    ...
    useEffect(() => {
        if (!profileLoading) {
            ...
            // Skip redirect if the user just paid on this page
            if (hasJustPaid.current) return;
    ...
    const handleUpiSubmit = async (e: React.FormEvent) => {
        ...
        hasJustPaid.current = true; // Prevent redirect after profile refresh
        try {
            ...
            // Show QR prompt first, then feedback
    ...
    // --- Post-Payment Step 1: QR Widget Prompt ---
    if (showQrPrompt) {
        const standaloneUrl = profile?.profile_slug
            ? `/qr-widget.html?slug=${encodeURIComponent(profile.profile_slug)}&name=${encodeURIComponent(profile.full_name || 'My Profile')}`
            : null;
        ...
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
                <div className="bg-zinc-900 rounded-2xl border border-zinc-800 w-full max-w-sm p-6 shadow-2xl relative" style={{ animation: 'fadeSlideUp 0.4s ease-out' }}>
                    {/* Success Badge */}
                    <div className="text-center mb-5">
                        <div className="mx-auto flex items-center justify-center h-14 w-14 rounded-full bg-green-900/30 mb-3 border border-green-500/20">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                            </svg>
                        </div>
                        <h2 className="text-xl font-bold text-white mb-1">Payment Received!</h2>
                        <p className="text-xs text-zinc-500">Your NFC card will be processed shortly.</p>
                    </div>

                    {/* QR Prompt Card */}
                    <div className="bg-zinc-950 rounded-xl border border-zinc-800 p-4 mb-5">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h3 className="text-sm font-bold text-white mb-1">Add QR to Home Screen?</h3>
                                <p className="text-xs text-zinc-400 leading-relaxed">
                                    Get a 1-tap shortcut to your profile QR code — like a Paytm-style QR screen. Works offline too!
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2.5">
                        <button
                            onClick={handleQrYes}
                            className="w-full py-3 px-4 bg-gold text-black font-bold rounded-xl hover:bg-gold-600 transition-all duration-200 shadow-lg shadow-gold/10 text-sm flex items-center justify-center gap-2"
                        >
    ...
    // --- Post-Payment Step 2: Feedback Modal ---
